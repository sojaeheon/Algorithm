...
## note-31-dfs-memoization-recursion-limit

### DFS 메모이제이션과 재귀 제한

DFS로 부분 문제를 계산하고 결과를 저장해 재사용하는 방식은 Top-down DP이다.

1024 내리막 길의 부분 문제는 다음과 같다.

```python
dfs(row, col) = (row, col)에서 도착점까지 가는 내리막 경로의 수
```

현재 칸의 답은 이동 가능한 모든 낮은 이웃 칸의 경로 수를 더해서 구한다.

```python
memo[row][col] += dfs(next_row, next_col)
```

도착점에서는 경로 하나가 완성됐으므로 `1`을 반환한다.
...
`memo`는 다음 세 상태를 구분한다.

| 값 | 의미 |
| ---: | --- |
| `-1` | 아직 계산하지 않은 칸 |
| `0` | 계산했지만 도착점까지 가는 경로가 없는 칸 |
| `1` 이상 | 도착점까지 가는 경로 수 |

이미 계산한 칸은 다시 탐색하지 않는다.
...
이동할 때마다 높이가 엄격하게 감소하므로 사이클이 없다.
...
Python의 기본 재귀 한도는 깊은 DFS에 부족할 수 있지만, `sys.setrecursionlimit()`을 무조건 매우 크게 설정해서는 안 된다.

```python
sys.setrecursionlimit(1_000_000)  # 입력과 무관한 과도한 설정
```

재귀 한도는 메모리를 늘리는 기능이 아니다. 호출이 깊어지면 함수 프레임이 계속 쌓이므로 과도한 설정은 메모리 초과나 런타임 오류를 일으킬 수 있다.

1024번은 높이가 10,000 이하이고 매번 더 낮은 칸으로 이동하므로 한 경로의 깊이도 최대 10,000 이하이다. 따라서 문제 조건에 맞는 수준으로만 설정한다.

```python
sys.setrecursionlimit(12_000)
```

실전에서는 최대 깊이를 계산하고 약간의 여유만 두는 것이 원칙이다.

#### 위상정렬 Bottom-up DP

높은 칸에서 낮은 칸으로 방향 간선을 만들면 사이클이 없는 DAG가 된다. 진입 차수가 0인 칸부터 처리하면서 경로 수를 낮은 칸으로 전달할 수 있다.
...
| 방식 | 시간 | 추가 특징 |
| --- | ---: | --- |
| DFS + 메모이제이션 | `O(NM)` | 코드가 자연스럽고 추가 배열이 적지만 재귀 깊이를 고려해야 함 |
| 위상정렬 + DP | `O(NM)` | 재귀가 없지만 `indegree`, `dp`, 큐가 필요함 |
| 높이순 정렬 DP | `O(NM log(NM))` | 재귀가 없지만 정렬 비용이 추가됨 |

64MB처럼 메모리가 작은 환경에서는 DFS 메모이제이션이 추가 배열이 적어 유리할 수 있다. 반면 최대 깊이가 수만 이상이거나 예측하기 어렵다면 반복 방식을 선택하는 편이 안전하다.
...
## problem-1024-downhill-path

### 1024 내리막 길

문제 파일: [1024_DownhillPath.py](gold/1024_DownhillPath.py)

배운 내용:

| 주제 | 이유 |
| --- | --- |
| [DFS 메모이제이션과 재귀 제한](#note-31-dfs-memoization-recursion-limit) | DFS가 부분 문제 계산 순서를 만들고 칸별 결과를 저장하는 Top-down DP임을 이해 |
| `dfs(row, col)` | 현재 칸에서 도착점까지 가는 경로 수를 반환 |
| `memo`의 `-1`, `0`, 양수 | 미계산 상태와 계산했지만 경로가 없는 상태를 구분 |
| 도착점에서 `return 1` | 이동 횟수가 아니라 완성된 경로 하나를 상위 호출에 전달 |
| 엄격한 높이 감소 | 사이클이 생기지 않으므로 칸별 결과를 안전하게 메모이제이션 |
