...
#   전체 동전에서 물건값 W를 뺀 금액을 가능한 적은 개수의 동전으로 남기면,
#   사용한 동전 개수는 자동으로 최대가 된다.
# 시간 복잡도: O(1)
#   동전 종류가 500, 100, 50, 10, 5, 1로 항상 6개라 반복 횟수가 고정이다.
# 공간 복잡도: O(1)
#   동전 종류 수가 고정이므로 사용하는 배열 크기도 항상 일정하다.
